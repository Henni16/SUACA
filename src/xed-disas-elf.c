...
disas_test32(xed_disas_info_t* fi,
	     ...
	     Elf32_Word size, 
             ...
             xed_symbol_table_t* symbol_table, inst_list_t* instructions)
{
  ...
  xed_disas_test(fi, instructions);
}
              ...
              xed_symbol_table_t* symbol_table, inst_list_t* instructions)
{
    ...
    for( i=0;i<nsect;i++)  {
        ...
        if (text) {
            ...
            disas_test64(fi, 
                         start, length, shp[i].sh_offset, shp[i].sh_size, 
                         shp[i].sh_addr, symbol_table, instructions);
        }
    }
}
...
void
xed_disas_elf(xed_disas_info_t* fi, inst_list_t* instructions)
{
    void* region = 0;
    unsigned int len = 0;
    ...
    if (check_binary_64b(region)) {
        ...
        process_elf64(fi, region, len, &symbol_table, instructions);
    }
    else if (check_binary_32b(region)) {
        symbols_elf32(fi, region, len,  &symbol_table);
        process_elf32(fi, region, len, &symbol_table, instructions);
    }
    else {
