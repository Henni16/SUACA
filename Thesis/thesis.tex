    ...
    \While{loaded\_\microops(Instruction) $> 0$} {
        \eIf{$!all\_fathers\_done(Instruction)$} {
            ...
            \eIf{Executable} {
                ...
                Blamed $:=$ Set of instructions\;
                \ForEach{p $\in$ usable\_ports(Instruction)} {
                    \If{p.using\_instruction() $\neq$ Instruction \textbf{and} !Blamed.contains(p.using\_instruction())} {
...
\begin{algorithm}[H]
    ...
    \ForEach{p $\in$ PortQueue} {
        \uIf{$\mu$.can\_use(p) \textbf{and} p.is\_free()} {
            p.uses($\mu$)\;
            \Return $true$\;
        }
        \ElseIf{$\mu$.can\_use(p) \textbf{and} p.using\_instruction() $\neq I$} {
...
If all dependencies have been resolved \suaca\ will try to assign all \microops\ of the instruction to a port. The function \textbf{\emph{assign\_to\_ports(\microop, Instruction)}} is described in \autoref{alg:assign}. This function will iterate over all ports in prior usage order and assign the \microop\ if possible. More precisely \emph{PortQueue} contains all ports and is sorted by usage throughout the whole simulation. If possible it will assign the \microop\ to the port and return a success. If no usable port was free it will return a fail unless all ports were blocked by the instruction itself.\\
...
If the assignment of a single \microop\ failed the flag \emph{Executable} will be set to \emph{false}. As we can see in \autoref{line:if} the instruction will only added to the execution list (which we will further discuss in \autoref{sec:execute}) if this flag is set. This means that an instruction is only executed if all of its \microops\ could be assigned to a port. We are doing this, because of our measurements. Those will run the instructions ``in a vacuum'', i.e.\ there simply are no other instructions that might block a port (the instruction itself however might do so, that's why we are excluding this case in the \emph{assign\_to\_ports} function). So we will always measure the best case latency for every instruction. As we don't know if there is an order in which the instructions \microops\ have to be executed, or if they can be executed simultaneously, we have to assume a delay as soon as a single \microop\ is denied. As we need some more information about the actual execution of our instructions we will further discuss this in \autoref{sec:latency}.\\
...
\section{Computation of latency}
\label{sec:latency}
